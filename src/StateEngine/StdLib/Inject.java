...
public class Inject extends StandardUnaryFunction
{
    ...
    public ValueType fun(CallingContext context, ValueType arg) throws TypedOperationException, FatalException
    {
        if (arg instanceof StringValue)
        {
            ...
            LinkedList<State> inject = new LinkedList<State>();
            inject.add(new State(state));
            context.cell.machine.states.add(context.cell.machine.states.size() - 1, inject);
        }
        else if (arg instanceof ArrayValue)
        {
            ...
            for (ValueType val : value)
            {
                ...
                {
                    throw new TypedOperationException("Inject called with something other than string in list of names of states.");
                }
            }
            context.cell.machine.states.add(context.cell.machine.states.size() - 1, inject);
        }
        ...
        {
            throw new TypedOperationException("Inject called without string or list of string name of state(s).");
        }
